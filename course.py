...
class CourseClass:
    def __init__(self, root):
        ...
        title=Label(self.root,text="Course Details",font=("Times New Roman",15,"bold"),bg="#033054",fg="white").place(x=10,y=15,width=1180,height=35)
        ...
        txt_description=Entry(self.root,textvariable=self.var_decription,font=("Times New Roman",15,"bold"),bg="lightyellow").place(x=150,y=180,width=200)
        #self.txt_description=Text(self.root, width=20, height=3)
        ...
        txt_search_courseName=Entry(self.root,textvariable=self.var_search,font=("Times New Roman",15,"bold"),bg="lightyellow").place(x=870,y=60,width=180)
        ...
        self.CourseTable=ttk.Treeview(self.C_Frame,columns=("cid","name","duration","charges","description"),xscrollcommand=scrollz.set,yscrollcommand=scrolly.set)
    ...
    def delete(self):
        ...
        try:
            if self.var_course.get()=="":
                ...
                row=cur.fetchone()
                if row==None:
                    messagebox.showerror("Error","Pleasde select the course from list",parent=self.root)
                else:
    ...
    def get_data(self,ev):
        ...
        self.var_decription.set(row[4])
        #self.var_decription.insert(row)
    ...
    def add(self):
        ...
        try:
            if self.var_course.get()=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Coure name already present",parent=self.root)
                else:
                    cur.execute("insert into course(name,duration,charges,description) values(?,?,?,?)",(
                        self.var_course.get(),
                        self.var_duration.get(),
                        self.var_charges.get(),
                        self.var_decription.get()
                    ))
                    con.commit()
